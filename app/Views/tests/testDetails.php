...
<p class="mt-4 mb-3"><span class="card-text text-muted fw-bolder mt-3">Difficulty:&nbsp<span class="text-muted fw-normal"><?=$difficulty -> name?></span></span></br>
<span class="card-text text-muted fw-bolder">Max attempts:&nbsp<span class="text-muted fw-normal"><?=$test -> max_attempts?></span></span></p>
<?php
echo '<p>'.$timeAttempt.'</p>';
echo '<p>'.$testAttempt.'</p>';
if (count($attempts) <  $test -> max_attempts){
    if ($test->required_password == 1){
        ...
        echo '</div>';
        echo form_close();
    } else{ echo anchor('test-start/' . $test->id, 'Start', 'class="btn btn-primary"');}
} else {
...
if (count($attempts) != 0){
    $table = new Table();
    $table->setHeading('NUMBER','STARTED', 'FINISHED', 'SCORE');
